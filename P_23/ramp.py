...
for i in range(1,11):
    ...
    npt = md.integrate.npt(group = groupNP_mes, kT = temp, tau = 3.0, tauP = 3.0, P = p_max)
    ...
    log = analyze.log(filename = log_file, quantities = ['num_particles',
                                     ...
                                     'temperature',
                                     'pressure',
                                      'volume'], period = 1e2, overwrite = True)
    gsd = dump.gsd(gsd_file, period = 1e2, group = group.all(), overwrite = True); 
    meso_gsd = dump.gsd(meso_gsd_file, period = 1e2, group = groupNP_mes, overwrite = True); 
